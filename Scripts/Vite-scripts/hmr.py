...
# Target file: most-imported module (125+ dependents) to stress-test HMR propagation
...
def measure_hmr(process, target_path, ansi_escape):
    """
    Modify a file and measure wall-clock time until Vite reports HMR update.
    ...
    # Wait for Vite to finish server-side HMR processing.
    # Debug output sequence: [file change] -> (client) [update/no modules matched]
    # We time until the (client) line, which is when Vite has completed module-graph
    # invalidation and decided what to send the browser — comparable to Webpack's
    ...
    try:
        while True:
            ...
            if '(client)' in clean_line and 'Icon' in clean_line:
                ...
                # Wait for revert HMR to complete before next measurement
                ...
                while time.time() < deadline:
                    ...
                    if '(client)' in clean_revert and 'Icon' in clean_revert:
                        break
...
def main():
    print("Vite HMR measurements (10 runs)\n")

    script_dir = os.path.dirname(os.path.abspath(__file__))
    vite_dir = os.path.join(script_dir, '..', '..', 'Vite-app', 'Vite-app')
    target_path = os.path.join(vite_dir, TARGET_FILE)
    ...
    # Start the dev server with HMR debug logging enabled
    # Vite 7 suppresses HMR messages in non-TTY (piped) mode;
    # DEBUG=vite:hmr forces them to stderr which we merge into stdout.
    env = os.environ.copy()
    env['DEBUG'] = 'vite:hmr'
