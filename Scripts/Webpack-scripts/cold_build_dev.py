...
def run_cold_dev_server():
    """
    Run a cold dev server startup and extract compilation time.
    ...
    try:
        ...
        # Read output until we see compilation success
        for line in process.stdout:
            output += line
            clean_line = ansi_escape.sub('', line)

            # Look for "compiled successfully in X ms"
            if 'compiled successfully in' in clean_line:
                match = re.search(r'compiled successfully in (\d+) ms', clean_line)
                if match:
...
def main():
    ...
    for i in range(1, 11):
        ...
        kill_process_on_port(3000)  # Clean up port before starting server
